...
export default function JobDetailsModal({ job, onClose, userSkills }) {
  ...
  const isSkillBasedJob = job.job_tier === "skill_based";
  ...
  const handleSubmit = async (e) => {
    ...
    // For professional jobs, resume is required
    if (!isSkillBasedJob && !formData.resume_url) {
    ...
    try {
      ...
      // Only add resume_url for professional jobs
      if (!isSkillBasedJob) {
        applicationData.resume_url = formData.resume_url;
      }
  ...
  const hasMinSkills = isSkillBasedJob ? userSkills.length >= 1 : userSkills.length >= 3;
        ...
        <div className={`sticky top-0 p-6 rounded-t-3xl ${
          ...
          <div className="flex justify-between items-start">
            <div className="flex-1">
              ...
              <p className={`text-lg ${isSkillBasedJob ? "text-orange-100" : "text-purple-100"}`}>
                {job.company}
              </p>
            </div>
            <button
          ...
          {!showApplicationForm ? (
                  ...
                  {job.requirements?.map((req, index) => (
                    <li key={index} className="flex items-start gap-2 text-gray-700">
                      <CheckCircle2 className={`w-5 h-5 mt-0.5 flex-shrink-0 ${
                        isSkillBasedJob ? "text-orange-600" : "text-emerald-600"
                      }`} />
                      <span>{req}</span>
                    </li>
                  ))}
              ...
              <section className="mb-6">
                ...
                <ul className="space-y-2">
                  {job.responsibilities?.map((resp, index) => (
                    <li key={index} className="flex items-start gap-2 text-gray-700">
                      <CheckCircle2 className={`w-5 h-5 mt-0.5 flex-shrink-0 ${
                        isSkillBasedJob ? "text-orange-600" : "text-purple-600"
                      }`} />
                      <span>{resp}</span>
                    </li>
                  ))}
                ...
                {hasApplied ? (
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl text-base font-semibold transition-all ${
                      hasMinSkills
                        ? isSkillBasedJob
                          ? "bg-gradient-to-r from-orange-600 to-amber-600 text-white hover:shadow-lg hover:shadow-orange-500/50"
            ...
            <div className="text-center py-12">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 ${
                isSkillBasedJob ? "bg-orange-100" : "bg-emerald-100"
              }`}>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    {isSkillBasedJob ? "Years of Experience (can be 0) *" : "Years of Experience *"}
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    placeholder={isSkillBasedJob ? "0" : "3"}
                  />
                </div>

                {/* Resume upload only for professional jobs */}
                {!isSkillBasedJob && (
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isSkillBasedJob 
                      ? "Tell us about your experience with this skill..." 
                      : "Tell us why you're a great fit for this role..."
                    }
                  />
                </div>

                {/* Info message for skill-based jobs */}
                {isSkillBasedJob && (
                  <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                    <p className="text-sm text-orange-800">
                      ℹ️ <strong>No resume required!</strong> Just fill in your basic details and tell us about your experience.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={uploading || (!isSkillBasedJob && !formData.resume_url)}
